...
public class update_menu extends HttpServlet {
     ...
     boolean check(String name,String cost,String category,String description){
        ...
        if(!name.equals("")){
            ...
            return false;
        if(!category.equals("")){
           if(!category.equals("Soup")&&!category.equals("Salad")&&!category.equals("Starter")&&!category.equals("Sizzler")&&!category.equals("Pasta")&&!category.equals("Roll & Sandwich")&&!category.equals("Main Course")&&!category.equals("Dessert")&&!category.equals("Beverages"))
            return false;
        }
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(operation.equals("add")){
                ...
                if(check(name,cost,category,description))
                {
                    ...
                    int status=st.executeUpdate("insert into menu_items values("+id+",'"+name+"','"+cost+"','"+category+"','"+description+"')");
                    if(status==1){
                ...
                if(check2(name,cost))
                {
                    int status=st.executeUpdate("update menu_items set m_cost='"+cost+"' where m_name='"+name+"'");
                    if(status==1){
